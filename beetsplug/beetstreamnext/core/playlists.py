...
class Playlist:
    ...
    def make_id(dir_id, path: Path, owner: Optional[str]) -> str:
        ...
        return IDs.encode_playlist(dir_id, stem_suffix, owner)
    ...
    def load_songs(self) -> None:
        ...
        if id_entries:
            ...
            if stable_entries:
                resolved = Resolve.songs([e['props']['id'] for _, e in stable_entries])
        ...
        for idx in sorted(results):
            row = results[idx]
            self.songs.append(Serialise.song(row))
    ...
    def add_songs(self, beets_items) -> None:
        with self._lock:
            for item in beets_items:
                self.songs.append(Serialise.song(item))
            self._calc_duration()
            self.to_m3u()
    ...
    def from_songs(cls, name: str, songs: List['Item']) -> Playlist:
        ...
        instance.songs = [Serialise.song(song) for song in songs]
...
class PlaylistProvider:
    ...
    def get(self, playlist_id: str) -> Playlist | None:
        ...
        with self._lock:
            if IDs.decode_type(playlist_id) != 'playlist':
                return None
            ...
            try:
                parts = IDs.decode_playlist(playlist_id).split('-', 1)
                if len(parts) < 2:
                    return None
                dir_id = int(parts[0])
                rest = parts[1]
